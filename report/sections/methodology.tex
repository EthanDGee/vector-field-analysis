...
Connectivity is established by performing a \texttt{unite} operation between each cell and its calculated downstream neighbor.

The \texttt{unite} operation serves as the primary mechanism for establishing connectivity between grid cells.
In parallel contexts, this is performed using atomic synchronization primitives to ensure that multiple threads can safely update the shared set structure without requiring heavy-weight locks.
...
Boundary effects and terminal points are handled naturally: if a vector points back to its own cell (after clamping), it remains its own root or joins its existing set without further extension.
...
It is implemented using modern C++17 standards and processes the vector field grid in a single pass using nested loops to iterate through rows and columns in row-major order.
...
\begin{enumerate}
	...
	\item \textbf{Parallel Root Discovery:} After the sequential pass, the \texttt{findRoot} operation is performed in parallel for all cells to determine their final component assignments.
\end{enumerate}
...
The CUDA implementation offloads the computationally intensive tasks of neighbor calculation and component discovery to the GPU, leveraging its high throughput to handle millions of cells simultaneously.
...
\begin{enumerate}
	\item \textbf{Parallel Neighbor Precomputation:} A CUDA kernel assigns one thread to each grid cell to calculate its downstream successor.
